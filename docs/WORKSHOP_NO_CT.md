...
**Install prereqs at home, not in the room.** The first full pipeline run is ~4
minutes of rate-limited HTTP. A room full of people hitting CoinGecko from one
NAT will hit 429s and frustrate everyone.

This workshop calls each tool directly — `dlt` through a Python module, `dbt`,
`airflow`, DuckDB — so you see the real seams of the pipeline instead of a
wrapper script.
...
# 3. create the virtualenv from the lockfile
uv sync

# 4. activate it. This puts `dbt`, `airflow`, and the project `python` on PATH.
...
If `tables` shows non-zero rows in all four schemas (`raw`, `bronze`, `silver`,
...
**Step A — ingest (dlt).** A Python program that calls the APIs and lands raw
tables. dlt is a library, not a CLI: you run the project's own module.
...
dbt is downstream of dlt. Nothing in dbt can run before dlt has landed the raw
tables, which is exactly why they are two commands and not one.
...
python -m scripts.duckdb_cli sql "select count(*) from raw.coins_markets_raw"
```
...
3. Re-run the pipeline — two tools, in order:
   ```bash
   python -m ingest.run_ingest   # dlt: fetch the coin, land it in raw
